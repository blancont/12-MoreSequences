...
def run_test_generate_points_on_circle():
    ...
    expect = [rg.Point(210, 200),
              rg.Point(190, 200)]
    circ = rg.Circle(rg.Point(200, 200), 10)
    ans = generate_points_on_circle(circ, 2)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(300, 300, 'Test 4: 4 red dots')
    circ = rg.Circle(rg.Point(100, 100), 20)
    draw_points_on_circle(window, circ, 4, 'red')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(300, 300, 'Test 4:  100 slices, thin (thickness = 1) black lines')
    circ = rg.Circle(rg.Point(100, 100), 50)
    circ.fill_color = 'yellow'
    circ.outline_thickness = 1
    pizza(window, circ, 100, 'black', 1)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(500, 500, "Test 3: 4 segments w/ thin red lines")

    circle = rg.Circle(rg.Point(250, 250), 50)
    polygon(window, circle, 4, 'red', 1)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)):
        line = rg.Line(points[k], rg.Point(0, 0))
        line.color = color
        line.thickness = thickness
        if k != len(points) - 1:
            line.end = points[k + 1]
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(500, 500, "Test 4: 300 white lines on black circle, hops = 61")
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        ...
        if k + hops_to_next_point >= len(points):
            line.end = points[k + hops_to_next_point - len(points)]
        else:
            line.end = points[k + hops_to_next_point]
        line.attach_to(window)
